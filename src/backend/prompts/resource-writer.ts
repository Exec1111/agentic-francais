/**
 * Prompts de l'Agent RessourceWriter
 * Quatre variantes selon (type de ressource) × (présence ou absence d'un texte corpus) :
 *  - extrait_oeuvre / oeuvre_complete  avec corpus → présentation pédagogique fidèle
 *  - exercice                          avec corpus → exercices basés sur le texte exact
 *  - cours / bilan                     avec corpus → document ancré dans le texte étudié
 *  - toutes ressources                 sans corpus → génération libre
 */

import type { LLMMessage } from '../llm-provider'
import type { getCorpusById } from '../repositories/corpus-repo'

export interface ResourceWriterOptions {
...
export function buildResourceMessages(opts: ResourceWriterOptions): LLMMessage[] {
  const {
...
${activiteTitle ? `- Activité : "${activiteTitle}" (Type : ${activiteType || 'Exercice'})` : ''}`
  ...
  if (corpusItem && (ressourceType === 'extrait_oeuvre' || ressourceType === 'oeuvre_complete')) {
    return buildExtractOeuvreMessages(corpusItem, contextBlock, ressourceTitle, niveau)
  }

  if (corpusItem && ressourceType === 'exercice') {
    return buildExerciceCorpusMessages(corpusItem, contextBlock, ressourceTitle, ressourceType, formatExercice, niveau)
  }

  if (corpusItem && (ressourceType === 'cours' || ressourceType === 'bilan')) {
    return buildCoursCorpusMessages(corpusItem, contextBlock, ressourceTitle, ressourceType, niveau)
  }

  return buildFreeMessages(contextBlock, ressourceTitle, ressourceType, formatExercice, niveau)
}

// ── Variante : extrait ou œuvre avec corpus ───────────────────────────────────

function buildExtractOeuvreMessages(
  corpusItem: NonNullable<Awaited<ReturnType<typeof getCorpusById>>>,
  contextBlock: string,
  ressourceTitle: string,
  niveau: string,
): LLMMessage[] {
...
// ── Variante : exercice ancré dans un texte corpus ────────────────────────────

function buildExerciceCorpusMessages(
  corpusItem: NonNullable<Awaited<ReturnType<typeof getCorpusById>>>,
  contextBlock: string,
  ressourceTitle: string,
  ressourceType: string,
  formatExercice: string | undefined,
  niveau: string,
): LLMMessage[] {
...
// ── Variante : cours ou bilan avec référence corpus ───────────────────────────

function buildCoursCorpusMessages(
  corpusItem: NonNullable<Awaited<ReturnType<typeof getCorpusById>>>,
  contextBlock: string,
  ressourceTitle: string,
  ressourceType: string,
...
// ── Variante : sans corpus (comportement original / libre) ────────────────────

function buildFreeMessages(
  contextBlock: string,
  ressourceTitle: string,
  ressourceType: string,
  formatExercice: string | undefined,
