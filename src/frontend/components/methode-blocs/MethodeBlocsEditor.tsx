...
interface Props {
  ...
  onChange: (next: MethodeContenu) => void
}

const BLOC_TYPES: MethodeBlocType[] = ['etape', 'titre_section', 'paragraphe', 'exemple', 'encadre', 'liste']
...
export function MethodeBlocsEditor({ contenu, onChange }: Props) {
  const newId = () =>
    typeof crypto !== 'undefined' && crypto.randomUUID ? `b-${crypto.randomUUID().slice(0, 8)}` : `b-${Date.now()}`

  const patch = useCallback(
    (changes: Partial<MethodeContenu>) => onChange({ ...contenu, ...changes }),
    [contenu, onChange]
  )
  const updateBloc = useCallback(
    (id: string, changes: Partial<MethodeBloc>) =>
  ...
  const addBloc = useCallback(
    (type: MethodeBlocType) => patch({ blocs: [...contenu.blocs, createEmptyMethodeBloc(type, newId())] }),
    [contenu.blocs, patch]
  )
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-gray-800 bg-gray-900/30 p-4 space-y-3">
        <div>
          <label className={labelCls}>Titre de la méthode</label>
          <input className={inputCls} value={contenu.titre}
            onChange={(e) => patch({ titre: e.target.value })} placeholder="Ex : Rédiger un paragraphe argumenté" />
        </div>
        <div>
          <label className={labelCls}>Objectif (optionnel)</label>
          <input className={inputCls} value={contenu.objectif ?? ''}
            onChange={(e) => patch({ objectif: e.target.value || null })} placeholder="À quoi sert cette méthode" />
        </div>
        <div>
          ...
          <textarea className={cn(inputCls, 'resize-y min-h-[40px]')} value={contenu.note_prof_globale ?? ''}
            ...
            placeholder="Conseils de mise en œuvre (masqué pour l'élève)" />
        </div>
      </div>
...
function BlocCard({
  ...
  index: number
  total: number
  onUpdate: (changes: Partial<MethodeBloc>) => void
  onMove: (dir: -1 | 1) => void
  onRemove: () => void
}) {
  return (
...
function BlocForm({ bloc, onUpdate }: { bloc: MethodeBloc; onUpdate: (c: Partial<MethodeBloc>) => void }) {
  switch (bloc.type) {
    case 'titre_section':
      return (
        <div>
          ...
          <input className={inputCls} value={bloc.texte ?? ''}
            onChange={(e) => onUpdate({ texte: e.target.value })} placeholder="Ex : Avant de rédiger" />
        </div>
      )
    ...
    case 'etape':
      return (
        <>
          <div>
            <label className={labelCls}>Titre de l&apos;étape</label>
            <input className={inputCls} value={bloc.titre ?? ''}
              onChange={(e) => onUpdate({ titre: e.target.value })} placeholder="Ex : Annoncer l'idée principale" />
          </div>
          <div>
            <label className={labelCls}>Description</label>
            <textarea className={cn(inputCls, 'resize-y min-h-[60px]')} value={bloc.texte ?? ''}
              onChange={(e) => onUpdate({ texte: e.target.value })} placeholder="Ce qu'il faut faire concrètement" />
          </div>
        </>
      )
      ...
      return (
        <div>
          ...
          <textarea className={cn(inputCls, 'resize-y min-h-[70px]')} value={bloc.texte ?? ''}
            onChange={(e) => onUpdate({ texte: e.target.value })} placeholder="Texte explicatif" />
        </div>
      )
      ...
      return (
        <div>
          ...
          <textarea className={cn(inputCls, 'resize-y min-h-[50px]')} value={bloc.texte ?? ''}
            onChange={(e) => onUpdate({ texte: e.target.value })} placeholder="Exemple concret d'application" />
        </div>
      )
        ...
        <>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className={inputCls} value={bloc.encadre_variante ?? 'astuce'}
            ...
            <div>
              ...
              <input className={inputCls} value={bloc.encadre_titre ?? ''}
                onChange={(e) => onUpdate({ encadre_titre: e.target.value })} placeholder="Ex : Astuce" />
            </div>
          </div>
          <div>
...
function ListeForm({ bloc, onUpdate }: { bloc: MethodeBloc; onUpdate: (c: Partial<MethodeBloc>) => void }) {
