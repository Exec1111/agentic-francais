...
import { FolderOpen, Trash2, Clock, BookOpen } from 'lucide-react'
...
export function SavedSequences({ sequences, loading, onLoad, onDelete, onRefresh }: Props) {
  useEffect(() => {
    onRefresh()
  }, [onRefresh])

  if (sequences.length === 0 && !loading) return null
  ...
  return (
    <div className="bg-gray-900/50 rounded-xl border border-gray-800 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-gray-600">({sequences.length})</span>
      </div>
      ...
      <AnimatePresence>
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {sequences.map((seq) => (
            ...
            >
              <BookOpen className="h-3.5 w-3.5 text-gray-600 group-hover:text-primary-400 transition-colors shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-300 truncate group-hover:text-white transition-colors">
                  {seq.titre}
                </p>
                <p className="text-xs text-gray-600">
                  {seq.niveau} · {seq.theme} · {seq.seanceCount} séance{seq.seanceCount > 1 ? 's' : ''}
                </p>
              </div>
